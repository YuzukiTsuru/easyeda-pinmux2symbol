...
2. Импортируйте `build/dist/pinmux2symbol_v1.1.10.eext` в менеджере расширений EasyEDA Pro.
...
Первая строка должна содержать `Pin Name` и `IO Type`. Необязательный столбец `PIN` задаёт номер вывода корпуса (например, `D10` или `B5`), остальные столбцы считаются функциями мультиплексирования. Для старых CSV без `PIN` сохраняется последовательная нумерация. Файл [`reference/pinout.csv`](./reference/pinout.csv) из репозитория можно импортировать напрямую.

Созданный символ содержит:

- один уникально пронумерованный вывод на каждую строку CSV с номером корпуса из необязательного столбца `PIN`;
...
- текст таблиц Bank и MUX использует полужирный `Courier New`; атрибуты имени и номера вывода по умолчанию используют обычный `Arial`, `0.08 inch` и цвет `#A2A2A2`;
